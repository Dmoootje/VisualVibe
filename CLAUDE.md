...
- **Horizontal alignment (fixed requirement).** Every section's content box (hero included) shares ONE left/right edge with the nav and footer. That edge is the Tailwind `.container` (max-width 1400px; gutters: 10px on mobile, 1rem from `sm`). So each section's inner wrapper is `container mx-auto px-2.5 sm:px-4` (+ its own `grid`/spacing classes), with horizontal padding *inside* the max-width box - identical to the container config, so bare `.container` usages line up too. NEVER put horizontal padding on the outer `<section>` when it wraps a `.container` (that doubles the gutter), and NEVER use a different content width (`max-w-[1300px]`, `sm:px-8`, etc.) for the left-aligned content column, or the section drifts out of line with the hero/nav. Deliberately narrower, centered reading columns (FAQ) may use a smaller `max-w-*` with `mx-auto` since they stay symmetric. Full-width marquees (gemeente-runner, sector-badges) may deliberately full-bleed edge-to-edge on mobile via `relative left-1/2 w-screen -translate-x-1/2` inside a section with `overflow-x-clip`.
- Theme colors go through the CSS variables in `src/app/globals.css` - no raw hex values in components.
- **Banned pattern: gradient icon tiles.** Square/rounded-square tiles with a red-to-amber gradient fill and a white icon (`bg-gradient-to-r from-red-500 to-amber-500` on an icon chip) are NOT VisualVibe branding and must never be (re)introduced. Use the brand chip instead: subtle dark fill + orange border + orange icon (`border-[rgba(255,122,0,0.3)] bg-[rgba(255,122,0,0.12)] text-[#FF9A45]`, cf. `.rbento-chip`), and for service cards the bento card style (`.rbento-*`: icon-chip top-left, ghost watermark right, animated conic border). Gradient CTA *buttons* are fine - the ban is on icon tiles/chips.
- **Mobile buttons: full width.** Hero/CTA button rows stack vertically at full width below `sm` (`flex-col` + `w-full justify-center sm:w-auto`, or the `.vvsh-btn` mobile rules); never leave two half-width buttons side by side on a phone.
