...
function EnglishHomepageBody({ reviews }: { reviews: GoogleReview[] }) {
  ...
  return <>
    ...
    <section className="container mx-auto px-2.5 py-16 sm:px-4"><h2 className="text-3xl font-bold sm:text-4xl">Based in Limburg, working across the region</h2><p className="mt-4 max-w-3xl text-lg text-white/65">VisualVibe is based in Tongeren-Borgloon and works with businesses throughout Limburg, Flanders, Antwerp province and Dutch Limburg. We also travel further for larger projects.</p><NextLink href="/en/regions/" className="mt-6 inline-block font-semibold text-amber-400">View all regions</NextLink></section>
    <section className="container mx-auto px-2.5 py-16 sm:px-4"><h2 className="text-3xl font-bold sm:text-4xl">Who we work with</h2><p className="mt-4 max-w-3xl text-lg text-white/65">We adapt the strategy, production and delivery to your market, audience and sales journey.</p><div className="mt-8 flex flex-wrap gap-3">{["Construction and renovation", "Hospitality", "Property", "Retail", "Events", "Sport", "Education", "Wellness", "Industry"].map((sector) => <span key={sector} className="rounded-full border border-white/10 bg-white/[.03] px-5 py-3 font-semibold">{sector}</span>)}</div><NextLink href="/en/sectors/" className="mt-6 inline-block font-semibold text-amber-400">Explore the sectors we serve</NextLink></section>
    ...
    <EnglishTestimonials reviews={reviews} />
    <BlogPreview />
    <section className="container mx-auto px-2.5 py-16 text-center sm:px-4"><h2 className="text-3xl font-bold sm:text-4xl">Ready to discuss your project?</h2><p className="mx-auto mt-4 max-w-2xl text-white/65">Tell us what you need and we will explore the right combination of web design, search, photography, video or digital development.</p><NextLink href="/en/request-a-quotation/" className="mt-7 inline-block rounded-xl bg-gradient-to-r from-red-500 to-amber-500 px-7 py-4 font-bold">Request a quotation</NextLink></section>
  </>;
}
