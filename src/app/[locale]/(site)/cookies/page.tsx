...
function EnglishCookies({ settings }: { settings: Awaited<ReturnType<typeof getSiteSettings>> }) {
  ...
  <H2>2. Which cookies does this website use?</H2><p>We deliberately keep cookie use to a minimum. We do not display advertising or sell data. We use two types of cookies:</p><ul className="list-disc space-y-2 pl-5"><li><strong className="text-white">Strictly necessary functional cookies:</strong> security and session cookies used only when a {settings.companyName} administrator signs in to the protected administration area. Regular visitors do not receive these cookies. These cookies are necessary for security and do not require consent.</li><li><strong className="text-white">Analytics cookies (Google Analytics 4):</strong> with your consent, we use Google Analytics to measure anonymously how visitors use the site, including viewed pages, device type and approximate region. Cookies include <code>_ga</code> and <code>_ga_&lt;id&gt;</code>. They are only stored after you accept them in the cookie banner. We do not use analytics data for advertising or profiling.</li></ul>
